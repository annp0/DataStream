...
import RIO.Vector.Unboxed as V
import RIO.Vector.Unboxed.Partial as V
import Data.Bits

{-# INLINABLE calcCRC #-}
calcCRC :: ByteString -> Word16
...
{-# INLINABLE checkCRC #-}
checkCRC :: ByteString -> Bool
checkCRC bs = 
  let (dat, binCrc) = B.splitAt (B.length bs - 2) bs 
      b0 = fromIntegral $ binCrc `B.index` 0 
      b1 = fromIntegral $ binCrc `B.index` 1 
      crc :: Word16 
      !crc = (b0 `shiftL` 8) .|. b1 
      !calculatedCRC = calcCRC dat 
  in 
    calculatedCRC == crc 
